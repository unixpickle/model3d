...
// JoinSDFs takes a union of SDFs using the maximum SDF value.
//
// Note that the result may no longer be a truly valid distance
// function.
func JoinSDFs(sdfs []SDF) SDF {
	min, max := BoundsUnion(sdfs)
...
// IntersectSDFS takes an intersection of SDFs using the
// minimum SDF value.
//
// Note that the result may no longer be a truly valid distance
// function.
func IntersectSDFs(sdfs []SDF) SDF {
	min, max := BoundsIntersection(sdfs)
	return FuncSDF(min, max, func(c Coord) float64 {
		val := sdfs[0].SDF(c)
		for _, s := range sdfs[1:] {
...
// SubtractSDF subtracts b from a using min(a, -b).
//
// Note that the result may no longer be a truly valid distance
// function.
...
func ClipSDF(s SDF, min, max Coord) SDF {
