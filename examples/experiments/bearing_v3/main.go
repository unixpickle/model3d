...
type Args struct {
	...
	RollerLargerRadius  float64 `default:"9"`
	RollerSmallerRadius float64 `default:"5"`
	RollerInOutHeight   float64 `default:"4"`
	RollerSpaceFrac     float64 `default:"0.5"`
	RollerGap           float64 `default:"0.2"`
	Delta               float64 `default:"0.25" help:"Meshification delta"`
	CageRollerRadius    float64 `default:"2"`
	CageRollerSlack     float64 `default:"0.8"`
	CageTopBottomMargin float64 `default:"1"`
	CageThickness       float64 `default:"1.5"`
	CageTopBottomSlack  float64 `default:"4"`
}
...
func main() {
	...
	var cageRollers model3d.JoinedSolid
	for i := 0; i < rollerCount; i++ {
		...
		cutout := Cylinder(
			XYZ(center.X, center.Y, 0),
			XYZ(center.X, center.Y, args.Thickness),
			args.CageRollerRadius+args.CageRollerSlack,
		)
		rollers = append(rollers, Sub3(baseRoller, cutout))
		cageRollers = append(cageRollers, Cylinder(
			XYZ(center.X, center.Y, -args.CageThickness),
			XYZ(center.X, center.Y, args.Thickness+args.CageTopBottomSlack+args.CageThickness),
			args.CageRollerRadius,
		))
	}

	cageBase := Sub3(
		Cylinder(Z(-args.CageThickness), Z(0), args.InnerRadius+args.CageRollerRadius+args.CageTopBottomMargin),
		Cylinder(Z(-args.CageThickness), Z(0), args.InnerRadius-args.CageRollerRadius-args.CageTopBottomMargin),
	)
	cageTop := Sub3(cageBase, cageRollers)
	cageBottom := Join3(cageBase, cageRollers)
	...
	cageBottomMesh := model3d.DualContour(cageBottom, args.Delta, true, false)
	cageBottomMesh = cageBottomMesh.EliminateCoplanar(1e-5)
	cageTopMesh := model3d.DualContour(cageTop, args.Delta, true, false)
	cageTopMesh = cageTopMesh.EliminateCoplanar(1e-5)
	cageBottomMesh.SaveGroupedSTL("cage_bottom.stl")
	cageTopMesh.SaveGroupedSTL("cage_top.stl")
}
