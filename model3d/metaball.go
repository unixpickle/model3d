...
func SignedMetaballSolid(f MetaballFalloffFunc, radiusThreshold float64, pos, neg []Metaball) Solid {
	balls := append(append([]Metaball{}, pos...), neg...)
	weights := make([]float64, len(balls))
	for i := range balls {
		if i < len(pos) {
			weights[i] = 1
		} else {
	...
	return WeightedMetaballSolid(f, radiusThreshold, balls, weights)
}

// WeightedMetaballSolid is like MetaballSolid but with
// separate coefficients for each metaball.
func WeightedMetaballSolid(
	f MetaballFalloffFunc,
	radiusThreshold float64,
	balls []Metaball,
	weights []float64,
) Solid {
	if len(balls) != len(weights) {
		panic("number of balls must match number of weights")
	}
	if f == nil {
	...
	posBalls := make([]Metaball, 0, len(balls))
	for i, mb := range balls {
		if weights[i] > 0 {
			posBalls = append(posBalls, mb)
		}
	}
	if len(posBalls) == 0 {
		return NewRect(Origin, Origin)
	}
	min, max := BoundsUnion(posBalls)
	...
	valueForOutset := func(x float64) float64 {
		...
		for i, mb := range balls {
			if weights[i] > 0 {
				sum += weights[i] * f(mb.MetaballDistBound(x))
			}
		}
		return sum
	}
	...
	return CheckedFuncSolid(
		...
		func(c Coord3D) bool {
			var sum float64
			for i, mb := range balls {
				sum += weights[i] * f(mb.MetaballField(c))
			}
...
func (s sdfMetaball) MetaballField(c Coord3D) float64 {
	return -s.SDF.SDF(c)
}
