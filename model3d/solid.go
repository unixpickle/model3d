...
func normalizeAngleRadians(a float64) float64 {
...
func angleDistanceCW(from, to float64) float64 {
	return normalizeAngleRadians(from - to)
}

// RevolveSolidRange revolves the solid around the Z axis
// with a start angle and a total number of radians.
func RevolveSolidRange(s model2d.Solid, angleRad float64, startRad float64) (Solid, error) {
	min2 := s.Min()
	max2 := s.Max()
	rMax := math.Max(math.Abs(min2.X), math.Abs(max2.X))
	min := XYZ(-rMax, -rMax, min2.Y)
	max := XYZ(rMax, rMax, max2.Y)

	angle := angleRad
	start := normalizeAngleRadians(startRad)
...
func RevolveSDF(s model2d.SDF) SDF {
	min2 := s.Min()
	max2 := s.Max()
	rMax := math.Max(math.Abs(min2.X), math.Abs(max2.X))
	min := XYZ(-rMax, -rMax, min2.Y)
